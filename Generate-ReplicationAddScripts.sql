...
(
    ...
    -- NOTE: pre_creation_cmd, schema_option, ins_cmd, upd_cmd, del_cmd, type
    -- are NOT stored in the distributor MSarticles table.
    -- They reside in sysarticles on the Publisher DB.
    -- sp_addarticle below uses server defaults for those parameters.
    -- Add them manually to the generated scripts if non-default values are needed.
);
...
SELECT
    ...
    + N'    @destination_table = N''' + REPLACE(ISNULL(ra.destination_object, ra.article), N'''', N'''''') + N''';'
    -- @type, @pre_creation_cmd, @schema_option, @ins_cmd, @upd_cmd, @del_cmd
    -- are not available in distributor MSarticles. Add from sysarticles on
    -- the Publisher if non-default values are required.
FROM #ResolvedArticles ra
GROUP BY
